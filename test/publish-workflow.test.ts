...
describe('README publishing instructions', () => {
  ...
  it('is honest that the first publish may have to be done by hand', () => {
    // npm's docs do not say whether a trusted publisher can be configured for a
    // name that has never been published. Saying so beats being surprised by it
    // mid-release.
    expect(readme).toMatch(/never been published/i);
