...
describe('CLI telemetry payload', () => {
  ...
  it('attributes events to the HASHED dev id once logged in, so they join server-side events', async () => {
    ...
    try {
      ...
      // Same scheme as src/posthog.ts hashDevId: first 16 hex of SHA-256(dev id).
      // Byte-identical output is the whole point — it is what joins a CLI event
      // and a /checkout event into one PostHog person.
      const expected = createHash('sha256').update(devId).digest('hex').slice(0, 16);
      expect(captures[0].distinct_id).toBe(expected);
      // The raw dev id never appears anywhere in the payload.
      expect(JSON.stringify(captures[0])).not.toContain(devId);
    } finally {
