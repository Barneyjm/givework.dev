...
async function main() {
  const np = await pool.query<{ id: string }>(`SELECT id FROM nonprofits WHERE name = $1`, [NP_NAME]);
  if (np.rowCount === 0) {
  ...
  const tasks = await pool.query<{ id: string }>(`SELECT id FROM tasks WHERE nonprofit_id = $1`, [npId]);
  ...
  console.log(`Deleted ${led.rowCount} ledger row(s), ${tsk.rowCount} task(s) (${taskIds.length} ids), ${npd.rowCount} nonprofit.`);
  ...
  if (bud.rowCount) console.log(`Reset ${DEV_HANDLE}'s budget spend/reserve to 0 (cap ${bud.rows[0].budget_cents}¢).`);
