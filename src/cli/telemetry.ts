...
// captured server-side (src/posthog-server.ts), attributed to the dev. Sending
 ...
 * install and nothing more. Once the volunteer logs in we prefer their dev id
 * (see distinctId), which is what joins CLI events to the server-side and
 * browser events for the same person.
 */
...
function distinctId(): string {
  ...
  // The dev token's subject is the dev id, which is the same distinct id the
  // server and the browser use. Decoding the JWT payload is enough — we are not
  // verifying it, just reading our own stored token to label our own events.
  if (cfg.token) {
    try {
