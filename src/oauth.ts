...
import { captureAfterResponse } from './posthog-server.js';
...
export async function upsertDev(
  ...
  // (see captureAfterResponse) — sign-in should not wait on a third-party POST.
...
oauthRoutes.get('/github/callback', async (c) => {
  try {
    ...
    if (created) {
      captureAfterResponse(c, devId, 'contributor_signed_up', {
        via: 'github_oauth',
